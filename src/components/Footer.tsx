  ...
  const content = (
    ...
    >
      <div className="relative flex items-center justify-between p-2.5 rounded-lg hover:bg-[#ea4c3c]/5 transition-colors duration-300">
        ...
        {isExternal && (
          <ArrowRight
            className={`w-3.5 h-3.5 transition-all duration-300 ${isHovered ? 'opacity-100 translate-x-0 text-[#ea4c3c]' : 'opacity-0 -translate-x-2'}`}
          />
        )}
      </div>
    </motion.div>
    ...
    const fetchRepoData = async () => {
      try {
        ...
        if (releaseResponse.ok) {
          ...
          console.warn(`Latest release API returned ${releaseResponse.status}, using fallback version.`);
          setLatestVersion("v2.6.5");
        }
      } catch (error) {
  ...
  return (
    <footer className="relative border-t border-border bg-card text-foreground overflow-hidden">
      ...
      <div className="container mx-auto px-6 py-12 md:py-16 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 mb-12 md:mb-16">
          ...
          >
            <div className="flex items-start gap-4">
              <motion.div
                className="w-14 h-14 rounded-xl flex-shrink-0 flex items-center justify-center bg-background shadow-sm border border-border"
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                   <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#ea4c3c]/10 text-[#ea4c3c] border border-[#ea4c3c]/20">
            ...
            <div className="flex flex-wrap gap-2" aria-label="Key features">
              {features.map((feature, index) => (
                <div
                  key={index}
                  className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-background text-muted-foreground border border-border text-xs font-medium"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <Button asChild className="h-11 px-6 rounded-xl transition-all bg-[#ea4c3c] hover:bg-[#d63e2c] text-white border-0 shadow-none">
                <Link to="/#download-section" onClick={onDownloadClick}>
                  <DownloadSimple className="w-4 h-4 mr-2" />
                  {t('footer.downloadXMCL')}
                </Link>
              </Button>
              <Button variant="outline" className="h-11 px-6 rounded-xl border-border bg-transparent hover:bg-[#ea4c3c] hover:border-[#ea4c3c] hover:text-white text-foreground transition-colors" onClick={() => window.open('https://github.com/voxelum/x-minecraft-launcher/releases', '_blank')}>
              ...
              >
                <div className="h-12 w-auto flex items-center justify-center grayscale group-hover:grayscale-0 transition-all duration-300">
                  {sponsor.logo ? (
                    ...
                    <span className="text-xl font-bold text-muted-foreground group-hover:text-[#ea4c3c] transition-colors">
                      {sponsor.name}
                    </span>
                  )}
                </div>
        ...
        {/* Bottom Bar */}
        <motion.div
          className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8 border-t border-border text-sm"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
        ...
        >
          ...
          <div className="flex items-center gap-6 text-slate-500 dark:text-slate-400">
             <div className="flex items-center gap-1.5">
               ...
               <Heart className="w-3.5 h-3.5 text-[#ea4c3c] fill-[#ea4c3c] animate-pulse" />
             ...
             <div className="flex gap-4">
                <span>{t('footer.launcherBy')} <span className="font-medium text-muted-foreground dark:text-[#ea4c3c]">CIO10</span></span>
                <span>{t('footer.websiteBy')} <span className="font-medium text-muted-foreground dark:text-[#ea4c3c]">Baneronetwo</span></span>
             </div>
          </div>
        </motion.div>
      </div>
