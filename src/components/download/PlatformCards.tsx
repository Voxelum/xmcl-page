...
export const PlatformCards: React.FC<PlatformCardsProps> = ({ platformAssets, onDownload }) => {
  ...
  const renderAssetGroup = (title: string, assets: GitHubAsset[]) => {
    ...
    return (
      <div className="mb-4 last:mb-0">
        <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 ml-1">{title}</h4>
        <div className="space-y-2">
          {assets.map((asset) => (
            <Button
              key={asset.id}
              variant="ghost"
              className="w-full justify-between bg-background hover:bg-accent text-foreground border border-border hover:border-[#ea4c3c] group/btn h-auto py-2 px-4 rounded-xl transition-all duration-300"
  ...
  return (
    <section className="py-10 md:py-20 px-4 bg-background border-t border-border">
      <div className="container mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-8 md:mb-16 text-foreground">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 max-w-6xl mx-auto">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="group relative p-6 md:p-8 rounded-3xl bg-card border border-border hover:border-[#ea4c3c] transition-colors duration-300 shadow-md"
          >
            <div className="relative z-10">
              <div className="w-14 h-14 rounded-2xl bg-background border border-border flex items-center justify-center mb-6 text-[#ea4c3c]">
                <Laptop className="w-7 h-7" />
              </div>
              <h3 className="text-2xl font-bold text-foreground mb-2">Windows</h3>
              <p className="text-muted-foreground mb-8 h-6">{t('downloadMessages.windowsDescription') || 'Windows 10/11'}</p>
          ...
          >
            <div className="relative z-10">
              ...
              <p className="text-muted-foreground mb-8 h-6">{t('downloadMessages.macosDescription') || 'macOS 11.0+'}</p>
          ...
          >
            <div className="relative z-10">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-2">Linux</h3>
              <p className="text-muted-foreground mb-8 h-6">{t('downloadMessages.linuxDescription') || 'Debian, RPM, AppImage'}</p>
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                ...
                <div className="mb-4 last:mb-0">
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 ml-1">{t('downloadMessages.packageManagers') || 'Package Managers'}</h4>
                    <div className="space-y-2">
