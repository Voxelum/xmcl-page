...
When installing XMCL or downloading many Minecraft versions, mods, resource packs, and shaders, your system drive (`C:`) can quickly run out of storage space.
...
### Option C: macOS (DMG)
- Drag `XMCL.app` from `/Applications` to any connected disk volume, or keep the app in `/Applications` while changing the Game Data Directory inside launcher settings to an external drive.
